...
void kirjaudu(void){
    ...
    FILE * tiedosto;
    char kortti[256], tunnusluku[256], oikeaTunnusluku[256];
    int kirjautuminen = 1;
    double saldo;
    do {
        printf("Anna kortin numero:\n");
        fgets( kortti, 256, stdin );

        if (kortti[ strlen(kortti)-1] == '\n'){
            kortti[ strlen(kortti)-1] = '\0';
        }
        else{
            tyhjennapuskuri();
        }
        ...
        strcat(kortti, ".tili");
        tiedosto = fopen(kortti, "r");

        if (tiedosto == NULL) {
            printf("Kyseistä tiliä ei löydy.\nYritä uudelleen\n");
        }
        else{
            kirjautuminen = 0;

            fgets( oikeaTunnusluku, 256, tiedosto );
            if (oikeaTunnusluku[strlen(oikeaTunnusluku)-1] == '\n')
                   oikeaTunnusluku[strlen(oikeaTunnusluku)-1] = '\0';

            printf("Anna tunnusluku:\n");
            fgets( tunnusluku, 256, stdin );
            ...
            if ((strcmp(oikeaTunnusluku, tunnusluku)) == 0) {
                    fscanf(tiedosto, "%lf", &saldo);
                    Saldo = saldo;
            }

            while(strcmp(oikeaTunnusluku, tunnusluku) != 0){
...
int rahojen_jako(int nosto){
    ...
    while (nosto){
        if (nosto > 1000){
            printf("Suurin kertanosto voi olla enintään 1000 euroa.\nAnna pienempi nostosumma\n");
            nosto = lueint();
        }
        else{
            switch(nosto % 50){
